...
<?php foreach($object->getLastVersions(10) as $version): ?>
  ...
  <dd class="item-information">
      <?php echo __('%time_ago% ago', array(
                          "%time_ago%" => time_ago_in_words($version->getVersionCreatedAt('U'))));?>
  </dd>
    ...
    <?php $diff = $object->compareVersions($version->getVersion()-1, $version->getVersion());
      ...
      <?php foreach($fields as $name => $field): ?>
        <?php if (isset($diff[sfInflector::camelize($name)])) : ?>
          <dd class="version-field"><?php echo __('Field <strong>%fieldlabel%</strong> changed', array(
                  '%fieldlabel%' => __($field->getConfig('label')))); ?></dd>
        <?php endif; ?>
      <?php endforeach; ?>
    <?php endforeach; ?>
